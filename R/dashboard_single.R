...
dashboard_single <- function(x, file = NULL, title = "Diagnostics") {
  ...
  make_table <- function(df) {
    ...
    rows <- lapply(seq_len(nrow(df)), function(i) {
      htmltools::tags$tr(
        lapply(df[i, , drop = TRUE], function(cell) {
  ...
  plot_to_img <- function(
    ...
    if (
      !file.exists(tf) || is.na(file.info(tf)$size) || file.info(tf)$size == 0
    ) {
      return(
        htmltools::tags$div(
  ...
  n_out <- if (is.null(out)) {
    NA
  } else if (is.null(nrow(out))) {
    NA
  } else {
    nrow(out)
  }
  ...
  p_seas <- suppressWarnings(as.numeric(extract_scalar(
    first_non_null(
      safe_get(
        x,
        c("output", "user_defined", "diagnostics.seas-si-combined"),
        NULL
      ),
  ...
  p_td <- suppressWarnings(as.numeric(extract_scalar(
    first_non_null(
      safe_get(
        x,
        c("output", "user_defined", "regression.td-ftest", "pvalue"),
        NULL
      ),
  ...
  p_seas_qs <- suppressWarnings(as.numeric(extract_scalar(
    first_non_null(
      safe_get(
        x,
        c("output", "user_defined", "diagnostics.seas-sa-qs", "pvalue"),
        NULL
      ),
  ...
  p_seas_f <- suppressWarnings(as.numeric(extract_scalar(
    first_non_null(
      safe_get(
        x,
        c("output", "user_defined", "diagnostics.seas-sa-f", "pvalue"),
        NULL
      ),
      safe_get(
        x,
  ...
  spec_orig_plot <- function() {
    render_persephone_plot(plotSpectrum(x, tsType = "original"))
  }
  spec_sa_plot <- function() {
    render_persephone_plot(plotSpectrum(x, tsType = "sa"))
  }
  # spec_res_plot  <- function() render_persephone_plot(plotSpectrum(x, tsType = "residuals"))# wird momentan noch nirgends verwendet
  resid_density_plot <- function() {
    render_persephone_plot(plotResiduals(x, which = "sreshist"))
  }
  acf_plot <- function() render_persephone_plot(plotResiduals(x, which = "acf"))
  pacf_plot <- function() {
    render_persephone_plot(plotResiduals(x, which = "pacf"))
  }
  resid_qq_plot <- function() {
    render_persephone_plot(plotResiduals(x, which = "nqq"))
  }
      ...
      htmltools::tags$div(
        ...
        card(
          ...
          htmltools::tags$div(
            ...
            metric_box(
              "Outliers",
              ifelse(is.na(n_out), "Undefined", as.character(n_out)),
              small = TRUE
            ),
