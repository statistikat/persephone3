...
#
# obj_x13 <- perX13(AirPassengers, template = "rsa3", context = my_context,
...
genTd <- function(
  ...
  dat <- lapply(hd, function(x) {
    if (substr(x, 1, 6) == "easter") {
      z <- paste(
        "as.Date(Easter(year = start(y)[1] : end(y)[1]))",
        substr(x, 7, nchar(x))
      )
      a <- eval(parse(text = z))
    } else {
  ...
  if (length(noneasterRel) > 0) {
    ...
    datNE <- data.frame(
      datNE,
      rep(weight[noneasterRel], each = 1 + lastYear - firstYear),
      stringsAsFactors = TRUE
    )
    datNE <- unique(datNE)
  ...
  dat <- data.frame(
    dat,
    rep(weight, each = 1 + lastYear - firstYear),
    stringsAsFactors = TRUE
  )
  dat <- unique(dat)
  ...
  td <- matrix(
    nrow = length(y),
    ncol = 6,
    dimnames = c(list(NULL, c(dNam[1:6])))
  )
  dd0 <- matrix(
    rep(0, 7),
    nrow = length(y),
    ncol = 7,
    dimnames = c(list(NULL, dNam))
  )
  ...
  for (ii in seq_along(eMeans)) {
    ...
    col <- rep(
      which(dNam %in% substr(eMeans[[ii]][, 3], 1, 3)),
      nrow(eMeans[[ii]])
    )
    val <- eMeans[[ii]][, 1]
  ...
  ltermMAll <- do.call(
    rbind,
    replicate((lastYear - firstYear + 1), ltermM, simplify = FALSE)
  )
  td1 <- td + ltermMAll
  ...
  td1 <- ts(
    matrix(td1, nrow = nrow(td1), ncol = ncol(td1)),
    start = c(firstYear, 1),
    frequency = 12
  )
  colnames(td1) <- c(
  ...
  td <- ts(
    matrix(td, nrow = nrow(td), ncol = ncol(td)),
    start = c(firstYear, 1),
    frequency = 12
  )
  colnames(td) <- c(
  ...
  dd <- ts(
    matrix(dd, nrow = nrow(dd), ncol = ncol(dd)),
    start = c(firstYear, 1),
    frequency = 12
  )
  colnames(dd) <- c(
