#' SI-Ratio Plot (Interactive)
#'
#' Interactive visualization of SI-ratios and seasonal factors by period using
#' highcharter. Supports zoom, pan, and hover tooltips for exploratory analysis.
#' For static plots suitable for publications, see [plotSiRatiosGgplot].
#'
#' @details
...
plotSiRatiosHc <- function(x) {
  if (inherits(x$output$decomposition, "JD3X11")) {
    ...
    d10_mean <- data.frame(
      cycleName = names(d10_by),
      d10Mean = as.vector(d10_by)
    )
    dat <- merge(
      data.frame(
        year = floor(stats::time(d10)),
  ...
  hc <- hc %>%
    ...
    // Funktion zum Finden des naechsten Index
