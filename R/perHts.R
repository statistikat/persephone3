...
#'              iterate = TRUE, usrdef.outliersEnabled = TRUE,
...
#' ht
#'
#' ht2 <- perHts(a = ht, b = objX13, method = "x13", iterate = FALSE,
#'               template = "rsa0", usrdef.outliersEnabled = TRUE,
#'               usrdef.outliersType = c("AO","LS","LS"),
#'               usrdef.outliersDate = c("1950-01-01","1955-04-01","1959-10-01"))
#' ht2$run()
#' ht2
...
hierarchicalTimeSeries <- R6::R6Class(
  ...
  public = list(
    ...
    initialize = function(method = c("tramoseats", "x13"),
     ...
     # browser()
      ...
      if (!is.null(list)) {
        ...
        # Hier abfangen wenn falsche spec-Block-Namen!!!
    ...
    updateParams = function(component = "", iterate = FALSE, ...) {
      # If a specific subcomponent is requested, forward the call to that subcomponent
      if (nzchar(component)) {
        root <- self$getComponent(component)
        root$updateParams(iterate = iterate, ...)   # subtree handles recursion itself
  ...
  private = list(
    ...
    updateParamsDirect = function(...) {
      ...
      freq <- frequency(private$ts_internal)
      private$updateFun(name = private$template, freq = freq, init_spec = private$params_internal, ...)
      invisible(NULL)
    },
...
#'
#' ht$updateParams(easter.enabled = TRUE, iterate = TRUE)
#'
...
perHts <- function(..., method = c("tramoseats", "x13"),
                   ...
                   list = NULL, weights = NULL, iterate = FALSE) {
