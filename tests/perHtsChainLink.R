...
hierarchicalTimeSeries <- R6::R6Class(
  ...
  public = list(
    ...
    initialize = function(...,
                          ...
                          chain_weights = NULL,
                          reference_year = NULL,
                          indirect_mode = c("standard", "chainlinked"),
                          recursive = TRUE,
                          iterate = TRUE,
                          speclist = NULL) {
      #browser()
      userdefined_default = character(0)
      ...
      private$indirect_mode <- match.arg(indirect_mode)
      ...
      private$chain_weights_internal <- private$normalize_chain_weights(
    ...
    run = function(verbose = FALSE) {
      ## 1) Run child components first
      lapply(self$components, function(component) {
        component$run(verbose = verbose)
      })

      ## 2) Run direct aggregate using the aggregated raw series
      ts_to_run <- if (!is.null(self$ts)) self$ts else private$ts_internal
      private$runDirect(ts_to_run)

      ## 3) Compute indirect result
      if (identical(private$indirect_mode, "chainlinked")) {
        private$adjusted_indirect_internal <- private$compute_adjusted_indirect_chainlinked()
      } else {
        private$adjusted_indirect_internal <- private$aggregate(
          self$components, self$weights, which = "adjustedIndirect"
        )
      }

      ## 4) Compute quality statistics for indirect aggregate
      private$quality_indirect_internal <- private$compute_quality_indirect(
        private$adjusted_indirect_internal
      )
  ...
  active = list(
    ...
    adjustedIndirect = function() {
      ...
      if (!is.null(private$adjusted_indirect_internal)) {
        return(private$adjusted_indirect_internal)
      }
    ...
    #' @field qualityIndirect diagnostics for the indirectly adjusted aggregate
    qualityIndirect = function() {
      private$quality_indirect_internal
    },
  ...
  private = list(
    ...
    chain_weights_internal = NULL,   # normalized chain weights
    aggregate_chain_internal = NULL, # optional cached aggregate chain factor
    reference_year = NULL,           # rereferencing year
    indirect_mode = "standard",      # standard | chainlinked
    adjusted_indirect_internal = NULL,
    quality_indirect_internal = NULL,
    ...
    normalize_chain_weights = function(chain_weights, components) {
      if (is.null(chain_weights)) {
        return(list(components = NULL, aggregate = NULL))
      }
      ...
      # Preferred structure:
      # chain_weights = list(
      #   components = list(a = ts(...), b = ts(...)),
      #   aggregate  = ts(...)
      # )
      if (is.list(chain_weights) && !is.null(chain_weights$components)) {
        cw_comp <- chain_weights$components
        if (is.null(names(cw_comp))) {
          names(cw_comp) <- component_names
        }
        if (!all(component_names %in% names(cw_comp))) {
          stop("chain_weights$components must contain one ts for each component.")
        }
        cw_comp <- cw_comp[component_names]
        return(list(
          components = cw_comp,
          aggregate = chain_weights$aggregate
        ))
      }

      # Allow mts with one column per component
      if ("mts" %in% class(chain_weights)) {
        if (ncol(chain_weights) != length(component_names)) {
          stop("If chain_weights is an mts object, ncol(chain_weights) must equal the number of components.")
        }
        cw_comp <- as.list(chain_weights)
        if (is.null(colnames(chain_weights))) {
          names(cw_comp) <- component_names
        } else {
          names(cw_comp) <- colnames(chain_weights)
          cw_comp <- cw_comp[component_names]
        }
        return(list(components = cw_comp, aggregate = NULL))
      }

      # Allow plain named list parallel to components
      if (is.list(chain_weights)) {
        if (is.null(names(chain_weights))) {
          names(chain_weights) <- component_names
        }
        if (!all(component_names %in% names(chain_weights))) {
          stop("If chain_weights is a list, it must contain one element for each component.")
        }
        return(list(
          components = chain_weights[component_names],
          aggregate = NULL
        ))
      }

      stop("Unsupported format for chain_weights.")
    },

    align_single_ts = function(w, target_ts) {
      if (is.null(w)) {
        return(NULL)
      }
      if (!inherits(w, "ts")) {
        stop("All chain weights must be 'ts' objects.")
      }

      if (startEndAsDecimal(end(w), freq = frequency(w)) <
          startEndAsDecimal(end(target_ts), freq = frequency(target_ts))) {
        return(ts(
          rep(tail(w, 1), length(target_ts)),
          start = start(target_ts),
          frequency = frequency(target_ts)
        ))
      } else {
        return(window(w, start = start(target_ts), end = end(target_ts)))
      }
    },

    align_weights_for_tss = function(tss, weights) {
      if (is.null(weights)) {
        return(NULL)
      }
      ...
      for (i in seq_along(tss)) {
        if (startEndAsDecimal(end(weights[, i]), freq = frequency(weights[, i])) <
          ...
          weights_ts[[i]] <- ts(
            rep(tail(weights[, i], 1), length(tss[[i]])),
            start = start(tss[[i]]),
    ...
    trim_na_ts = function(x) {
      if (is.null(x)) return(NULL)
      ok <- which(!is.na(as.numeric(x)))
      if (length(ok) == 0) return(NULL)
      window(x, start = time(x)[min(ok)], end = time(x)[max(ok)])
    },

    window_last_years = function(x, nyears = 3) {
      if (is.null(x)) return(NULL)
      n <- min(length(x), nyears * frequency(x))
      idx0 <- length(x) - n + 1
      window(x, start = time(x)[idx0])
    },

    unchain_ts = function(x, cw) {
      cw_aligned <- private$align_single_ts(cw, x)
      out <- x / cw_aligned
      out[!is.finite(out)] <- NA
      out
    },

    rechain_ts = function(x, cw) {
      cw_aligned <- private$align_single_ts(cw, x)
      out <- x * cw_aligned
      out[!is.finite(out)] <- NA
      out
    ...
    rereference_ts = function(x, reference_year, target = 100) {
      if (is.null(reference_year)) {
        return(x)
      }
      ref <- window(x,
                    start = c(reference_year, 1),
                    end   = c(reference_year, frequency(x)))
      base <- mean(as.numeric(ref), na.rm = TRUE)
      if (!is.finite(base) || base == 0) {
        stop("Could not rereference indirect aggregate: reference year mean is missing or zero.")
      }
      ts(as.numeric(x) / base * target,
         start = start(x),
         frequency = frequency(x))
    },

    derive_aggregate_chain_factor = function(component_chain_tss, weights) {
      weights_ts <- private$align_weights_for_tss(component_chain_tss, weights)
      private$aggregate_ts(component_chain_tss, weights_ts)
    },

    safe_test = function(fun) {
      tryCatch({
        x <- fun()
        if (inherits(x, "JD3_TEST")) {
          return(list(
            value = unname(x[1]),
            pvalue = unname(x[2]),
    ...
    compute_adjusted_indirect_chainlinked = function() {
      cw <- private$chain_weights_internal

      if (is.null(cw$components)) {
        stop("indirect_mode = 'chainlinked' requires chain_weights.")
      }

      # 1) collect the already seasonally adjusted components
      sa_components <- lapply(self$components, function(component) {
        component$adjusted
      })

      # 2) unchain each SA component
      sa_unchained <- Map(
        f  = private$unchain_ts,
        x  = sa_components,
        cw = cw$components
      )

      # 3) aggregate in unchained space
      weights_ts <- private$align_weights_for_tss(sa_unchained, self$weights)
      agg_unchained <- private$aggregate_ts(sa_unchained, weights_ts)

      # 4) choose aggregate chain factor
      agg_chain <- cw$aggregate
      if (is.null(agg_chain)) {
        # fallback if user did not provide an aggregate chain factor
        agg_chain <- private$derive_aggregate_chain_factor(cw$components, self$weights)
      }

      # 5) rechain aggregate
      agg_chainlinked <- private$rechain_ts(agg_unchained, agg_chain)

      # 6) rereference
      agg_chainlinked <- private$rereference_ts(
        x = agg_chainlinked,
    ...
    compute_quality_indirect = function(sa_indirect) {
      if (is.null(sa_indirect)) {
        return(NULL)
      }

      sa <- private$trim_na_ts(sa_indirect)
      if (is.null(sa)) {
        return(NULL)
      }

      y <- private$trim_na_ts(private$ts_internal)
      if (!is.null(y)) {
        y <- window(y, start = start(sa), end = end(sa))
      }

      # multiplicative irregular proxy
      irr <- NULL
      if (!is.null(y)) {
        irr <- y / sa
        irr[!is.finite(irr)] <- NA
        irr <- private$trim_na_ts(irr)
      }

      # "last" trading-days test -> last 8 years
      sa_last3 <- private$window_last_years(sa, 3)
      irr_last3 <- if (!is.null(irr)) private$window_last_years(irr, 3) else NULL

      list(
        regarima_residuals = list(
          normality = NA,
          independence = NA,
          note = "Not available for purely indirect aggregate because no aggregate RegArima residuals are estimated."
        ),

        proxy_irregular = if (!is.null(irr)) list(
          normality = private$safe_test(function() {
            rjd3toolkit::doornikhansen(as.numeric(irr))
          }),
          independence = private$safe_test(function() {
            rjd3toolkit::ljungbox(as.numeric(irr),
                                  lag = frequency(irr),
                                  k = 2)
          })
        ) else NULL,

        residual_seasonality = list(
          qs_sa = private$safe_test(function() {
            rjd3toolkit::seasonality_qs(sa)
          }),
          f_sa = private$safe_test(function() {
            rjd3toolkit::seasonality_f(sa)
          })
        ),

        combined_seasonality = list(
          comb_sa = private$safe_test(function() {
            rjd3toolkit::seasonality_combined(sa, mul = TRUE)
          }),
          comb_sa_last = private$safe_test(function() {
            rjd3toolkit::seasonality_combined(sa_last3, mul = TRUE)
          }),
          comb_i = if (!is.null(irr)) private$safe_test(function() {
            rjd3toolkit::seasonality_combined(irr, mul = FALSE)
          }) else NULL,
          comb_i_last = if (!is.null(irr_last3)) private$safe_test(function() {
            rjd3toolkit::seasonality_combined(irr_last3, mul = FALSE)
          }) else NULL
        ),

        residual_trading_days = list(
          td_sa_all = private$safe_test(function() {
            rjd3toolkit::td_f(sa, nyears = 0)
          }),
          td_sa_last = private$safe_test(function() {
            rjd3toolkit::td_f(sa, nyears = -8)
          })
        )
      )
    },
